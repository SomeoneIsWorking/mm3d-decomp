...
# Reuse the zelda3d NCCH/ExeFS parser (locates exefs/exheader offsets).
...
ZELDA3D_TOOLS_DIR = os.environ.get(
    "ZELDA3D_TOOLS",
    os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "tools"))
if not os.path.isdir(ZELDA3D_TOOLS_DIR):
    sys.exit("engine tools dir not found: %s (set ZELDA3D_TOOLS)" % ZELDA3D_TOOLS_DIR)
sys.path.insert(0, ZELDA3D_TOOLS_DIR)
